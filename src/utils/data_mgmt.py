...
def validate_img(config: dict) -> None:
    ...
    for dirs in os.listdir(PARENT_DIR):
        full_path_data_dirs = os.path.join(PARENT_DIR,dirs)
        for images in os.listdir(full_path_data_dirs):
            path_to_img = os.path.join(full_path_data_dirs,images)
            try:
